...
FillMenuResult FillSendMenu(
	...
	// TODO: improve, calc DisableAutoDeleteInContextMenu earlier, with peer data from local vars
	if (sending && !FakePasscode::DisableAutoDeleteInContextMenu()) {
		menu->addAction(
			tr::lng_send_autodelete_message(tr::now),
			[=] { action(
				         { .type = ActionType::DeleteWhenRead },
				         details
			      );
			    },
			&st::menuIconDelete);
